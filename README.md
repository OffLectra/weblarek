...
| Поле | Тип | Описание |
|------|-----|----------|
| `dataBayer` | `Partial<IBuyer>` | Частичные данные покупателя |

**Методы:**
| Метод | Параметры | Возвращает | Описание |
|-------|-----------|------------|----------|
| `setData` | `dataBayer: Partial<IBuyer>` | `void` | Сохраняет данные покупателя (частичное обновление) |
| `getData` | — | `Partial<IBuyer>` | Возвращает все данные покупателя |
| `clear` | — | `void` | Очищает все данные заказа |
| `validate` | — | `TBuyerErrors` | Валидирует данные, возвращает объект с ошибками |
| `isDataValid` | — | `boolean` | Проверяет, валидны ли все данные |
...
- `postOrder(order: IOrder): Promise<IOrder>` — выполняет POST-запрос на эндпоинт `/order`, отправляет данные заказа